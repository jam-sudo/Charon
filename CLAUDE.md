...
### 6j. CLint 3-Tier 전략 (Sprint 2부터 적용)

CLint의 근본적 예측 한계(SMILES만으로 AAFE < 2.0 달성 불가)를 인정하고, 3단계 폴백으로 안전 설계.

**Tier 1 — Experimental (권장):**
- `apply_overrides({"clint_uL_min_mg": {"value": X, "source": "experimental"}})`
- source="experimental", CI 없음 (고정값), 불확실성 전파에서 제외
- 보고서: "CLint: experimental value (recommended)"

**Tier 2 — Charon XGBoost (기본):**
- `ADMETPredictor().predict(smiles).clint_hepatocyte` → μL/min/10^6 cells
- scaffold CV R² ≈ 0.30, AAFE ≈ 2.5 (정직한 측정)
- flag="clint_tier2_ml; hepatocyte units; experimental value recommended"
- Conformal CI 필수 부착 (log-space)

**Tier 3 — Classification fallback (AD 밖):**
- Low (<10) / Med (10-50) / High (>50) μL/min/10^6 cells
- 확률 가중 MC 샘플링으로 전파
- CRITICAL flag: "CLint from classification; experimental measurement essential"
- Sprint 3 구현 예정 (현재 Sprint 2는 Tier 1+2만)

**절대 금지:**
- Omega 사전훈련 CLint 모델 직접 이식 (error cancellation 오염 리스크)
- Random CV 수치를 scaffold CV로 보고
- Hepatocyte CLint에 fu_inc 보정 적용
- CI 없이 단일 점 예측 반환

### 6k. Conformal Prediction 규칙 (Sprint 2부터 적용)

Charon은 **log-space symmetric intervals만 사용**. Omega의 linear/log 혼용 금지.

**보정 절차 (`ConformalPredictor.calibrate()`):**
1. 보정 세트 로드 (adme_reference.csv, 153 rows)
2. 각 화합물에 대해 ML 예측 → `|log10(pred/obs)|` 잔차 계산
3. 속성별 P90 = 90th percentile 잔차 저장 (`method="higher"` for conservative quantile)
4. Interval = `[pred / 10^P90, pred × 10^P90]`
5. 실제 커버리지 재계산 → 85% 미만이면 warning

**물리적 제약:**
- fup: 구간 상한 1.0으로 clip (bounded quantity)
- CLint: 구간 [0.1, 1000] 물리적 범위로 clip
- 점 예측이 구간 밖이면 구간이 점 예측을 bracket하도록 재조정

**데이터 누출 방지 (필수):**
- 훈련 세트에서 `adme_reference.csv`의 InChIKey-14 제외 (훈련 스크립트에서 강제)
- 보정 세트와 훈련 세트 중복 금지
- 19 compounds 자동 제외 확인 (`train_clint.py`, 유사하게 `train_fup.py`는 42개 제외)

**금지:**
- Linear-space intervals (음수 하한 발생 가능, Omega peff 버그)
- 훈련 세트와 보정 세트 공유
- 보정 세트를 meta-learner 튜닝 등 다른 목적 재사용
- 커버리지 85% 미만을 무시하고 배포
